...
#calculates the hamming code for an 8 bit value and returns it as a string
def hamming8(par):
	...
	#Invariant: the array is not at the requisite length yet
	for x in range (0, 12):
		if x == 0 or x == 1 or x == 3 or x == 7:
			...
			count = count + 1
	#hold the values for calculation of even parity
	parity1 = 0
	parity2 = 0
	parity3 = 0
	parity4 = 0
	#invariant: the parity variables are not yet fully assinged
	for x in range (0, 12):
		if x % 2 == 0:
	...
	#place the parity into the array, using the remainder division
	hammingArray[0] = parity1 % 2
	hammingArray[1] = parity2 % 2
	hammingArray[3] = parity3 % 2
	hammingArray[7] = parity4 % 2
	#put hammingArray into a string
	returner = '0'
	doneOnce = 0
	#Invariant: the string does not have all the values in it yet
	for x in range (0,12):
		if doneOnce == 0:
...
#calculates the hamming code for a 16 bit value and returns it as a string
def hamming16 (par):
	temp = 0 #used to hold a value to be written to the array
	tempArray = [0]#the array of values
	doneOnce = 0 #if the iteration has been done once
...
#initialization method, gets input from the user
def hammingMain():
	#getting user input
	strOut = 1
